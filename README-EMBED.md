# Event Booking System - Embed Guide

This guide explains how to embed the Event Booking System widgets into your existing website or admin dashboard.
...
<div id="event-calendar-widget"></div>
<script>
  (function() {
    ...
    document.getElementById('event-calendar-widget').appendChild(iframe);
  })();
</script>
```
...
- ✅ Click events to view/edit/approve/decline
...
<div id="event-booking-widget"></div>
<script>
  (function() {
    const iframe = document.createElement('iframe');
    ...
    iframe.style.borderRadius = '8px';
    iframe.style.overflow = 'hidden';
    document.getElementById('event-booking-widget').appendChild(iframe);
  })();
</script>
```
...
- ✅ Date & time picker with 30-min intervals
- ✅ Add-ons selection
...
<script>
  (function() {
    const container = document.getElementById('event-calendar-widget');
...
window.addEventListener('message', function(e) {
  if (e.data.type === 'resize') {
    const iframe = document.querySelector('#event-calendar-widget iframe');
    if (iframe) {
...
export default function CalendarPage() {
  ...
  return <div id="event-calendar-widget"></div>;
}
```
...
<div id="calendar-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:9999;">
  <div style="position:relative; width:90%; height:90%; margin:5% auto; background:white; border-radius:12px; overflow:hidden;">
    <button onclick="closeCalendarModal()" style="position:absolute; top:10px; right:10px; z-index:10;">✕</button>
    <div id="event-calendar-widget-modal" style="height:100%;"></div>
  </div>
</div>
...
<script>
  function openCalendarModal() {
    ...
    const container = document.getElementById('event-calendar-widget-modal');
